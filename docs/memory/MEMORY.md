...
- 本倉是 GitHub Pages **產出**（`index.html` / `articles.js` / `articles/`）。`nml-daily.html` 只做轉址到 `/#nml`，不再單獨報表。構建腳本在本機 `.workbuddy/tmp/build_dashboard.py`，**不在此 repo**。改前端必須改模板，否則每小時的 rebuild 會蓋掉。
...
- **排程（2026-09-15）**：Windows `AIReviewDailySite` 自 08-26 後不再直推 main；Cursor 日更 agent 開 PR 不 merge 就不發佈。**真正會更新網站的是 GitHub Actions** `.github/workflows/daily.yml`（UTC 每小時 `:17`，`17 * * * *`，`workflow_dispatch` 可手動）。公開倉在整點 cron 常被跳過，舊的「每 2 小時」實際會空 5–7 小時；改成每小時偏離整點後，內容大約每 1–2 小時更新一次。跑 `scripts/rebuild_daily.py` 後 **直接 commit push `main`** → Pages。不要再開只停在 PR 的日更。
- **預覽**：只在瀏覽器打開頁面即可。不要截圖、不要產出 walkthrough 圖片／影片。

## 自動化
- **現行發佈路徑**：GitHub Actions `Daily dashboard`（cron 每小時 `:17` UTC，或 Actions 頁手動 Run）→ `scripts/rebuild_daily.py`（列表 + 擷取 NML / AI HOT 全文 + **今日要聞** RSS 去重合併）→ push `main` → `pages-build-deployment`。Windows `AIReviewDailySite` 與 Cursor 日更 PR **不再依賴**。

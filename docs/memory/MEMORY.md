...
## 自動化
- **現行發佈路徑**：GitHub Actions `Daily dashboard`（cron 北京 07:00 與 13:00，或 Actions 頁手動 Run）→ `scripts/rebuild_daily.py`（列表 + 擷取 NML / AI HOT 全文 + **今日要聞** RSS 去重合併）→ push `main` → `pages-build-deployment`。Windows `AIReviewDailySite` 與 Cursor 日更 PR **不再依賴**。
